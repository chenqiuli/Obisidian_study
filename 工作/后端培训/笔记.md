...
##### 13、充血模型、贫血模型：
##### 14、abp七层模型：
- IFCA.XXX.Domain - 领域层（不能平级调用，必须保证数据完整性的写这里、业务的写这里）
- IFCA.XXX.EntityFrameworkCore - ef core数据访问层（仓储层、基础设施层、用来把实体转换为数据库字段）、
- IFCA.XXX.Application - 应用服务层（可以有的选择的时候写这里、增删查改）、
- IFCA.XXX.Web - web层（启动项目层）、
- IFCA.XXX.Domain.Shared - 领域共享层（放枚举，因为领域层和应用服务契约层可能都要用到）、
- IFCA.XXX.Application.Contracts -应用服务契约层（）、
- IFCA.XXX.HttpApi.Client - 动态Api代理层（不需要写代码）
